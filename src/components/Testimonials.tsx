  ...
  return (
    <div className="flex gap-1">
      {Array.from({ length: count }).map((_, i) => (
        <svg key={i} viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-cyan-500 dark:text-cyan-400">
...
export default function Testimonials() {
  return (
    <section id="testimonials" className="bg-gray-50 dark:bg-gray-900 py-24 sm:py-32 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="group bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 sm:p-8 hover:border-cyan-300 dark:hover:border-cyan-600/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-gray-100 dark:hover:shadow-cyan-950/20"
            >
              {/* Quote icon */}
              <svg
                viewBox="0 0 32 32"
                fill="currentColor"
                className="w-8 h-8 text-cyan-200 dark:text-cyan-800/60 mb-4"
              >
